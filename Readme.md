...
- [Try it for yourself : get the required tools](#try-it-for-yourself--get-the-required-tools)
  ...
  - [Finally, perform some database normalisation/sanitization](#finally-perform-some-database-normalisationsanitization)
- [Details : How it works](#details--how-it-works)
  - [Extracts the raw database using python and a bunch of PDF tools](#extracts-the-raw-database-using-python-and-a-bunch-of-pdf-tools)
    - [Patches the raw database with manual patches](#patches-the-raw-database-with-manual-patches)
    - [Run the database normalisation/sanitization tool](#run-the-database-normalisationsanitization-tool)
    - [Further analysis can be conducted using the dbanalysis tool](#further-analysis-can-be-conducted-using-the-dbanalysis-tool)
...
#                               |     |    Aggregate recipes in a single one (all_recipes.json)
#                               |     |     |
python -m Sources.dbextractor false false true
```
